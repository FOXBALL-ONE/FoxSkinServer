...
class YggdrasilService(
    ...
    init {
        ...
        require(properties.apiPath.length > 1 && !properties.apiPath.endsWith('/') &&
    ...
    fun hasJoined(username: String?, serverId: String?, @Suppress("UNUSED_PARAMETER") ip: String? = null): Map<String, Any>? {
    ...
    private fun invalidCredentials() = protocolError(HttpStatus.FORBIDDEN, "ForbiddenOperationException", "Invalid credentials. Invalid username or password.")
    private fun invalidToken() = protocolError(HttpStatus.FORBIDDEN, "ForbiddenOperationException", "Invalid token.")
    private fun expiredToken() = protocolError(HttpStatus.FORBIDDEN, "ForbiddenOperationException", "Token has expired.")
    private fun malformed(field: String) = protocolError(HttpStatus.BAD_REQUEST, "IllegalArgumentException", "Missing or invalid $field.")
    ...
    private fun loadKeyPair(): KeyPair {
        ...
        val publicKey = when (privateKey) {
            is java.security.interfaces.RSAPrivateCrtKey -> java.security.spec.RSAPublicKeySpec(privateKey.publicExponent, privateKey.modulus)
