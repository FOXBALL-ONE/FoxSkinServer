...
class JwtService(
    ...
    fun createAccessToken(user: AuthenticatedUser): String =
        createToken(user.userId, "access", properties.access.ttlSeconds)
    ...
    fun parse(token: kotlin.String): JwtClaims? {
        return try {
            ...
            val payload: Map<String, Any> =
                objectMapper.readValue(decoder.decode(parts[1]), object : TypeReference<Map<String, Any>>() {})
            ...
            // 缺失 iat 时按 0 处理：这类 token 会被任何一次凭据变更判定为过期，是更安全的默认值。
            val issuedAt = payload["iat"]?.toString()?.toLongOrNull() ?: 0L
            JwtClaims(subject, jti, type, issuedAt, expires)
        } catch (_: Exception) {
            null
        }
    }
